...
struct xz_dec {
	...
	/* CRC64 value in Block when the common XZ default check is used */
	uint64_t crc64;
...
static enum xz_ret XZ_FUNC dec_block(struct xz_dec *s, struct xz_buf *b)
{
	...
	if (s->check_type == XZ_CHECK_CRC32)
		...
		s->crc64 = uni_xz_crc64(b->out + s->out_start,
				b->out_pos - s->out_start, s->crc64);
...
/* Validate the little-endian CRC64 field used by default by xz-utils. */
static enum xz_ret XZ_FUNC crc64_validate(struct xz_dec *s, struct xz_buf *b)
{
	do {
		if (b->in_pos == b->in_size)
			return XZ_OK;

		if (((s->crc64 >> s->pos) & 0xFF) != b->in[b->in_pos++])
			return XZ_DATA_ERROR;
...
#else
	if (s->check_type != XZ_CHECK_NONE &&
			s->check_type != XZ_CHECK_CRC32 &&
			s->check_type != XZ_CHECK_CRC64)
		return XZ_OPTIONS_ERROR;
#endif
