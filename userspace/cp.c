...
static int copy_file(const char* src, const char* dst) {
    ...
    if (in_fd < 0) {
        write_cstr("cp: open source failed\n");
        return -1;
    }
    ...
    if (out_fd < 0) {
        ...
        write_cstr("cp: open destination failed\n");
        return -1;
    }
...
void aos_main(uint64_t argc, char** argv) {
    ...
    exit_code(copy_file(argv[1], argv[2]) == 0 ? 0 : 1);
}
