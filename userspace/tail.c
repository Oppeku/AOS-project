...
#define AT_FDCWD -100
#define O_RDONLY 0
#define TAIL_CAP 65536

static char tail_buf[TAIL_CAP];
...
static int tail_fd(int fd, uint64_t lines) {
    ...
    for (;;) {
        long rc = syscall3(SYS_READ, fd, (long)(tail_buf + len), TAIL_CAP - len);
        if (rc < 0) {
    ...
    if (skipped) {
        write_cstr("tail: input truncated to last 64 KiB buffer\n");
    }
    if (lines == 0 || len == 0) return 0;

    end = len;
    if (end > 0 && tail_buf[end - 1] == '\n') end--;
    start = end;
    ...
    if (found >= lines && tail_buf[start] == '\n') start++;

    if (write_all(tail_buf + start, len - start) != 0) {
        write_cstr("tail: write failed\n");
        return -1;
    }
    return 0;
}

static int tail_file(const char* path, uint64_t lines) {
...
void aos_main(uint64_t argc, char** argv) {
    ...
    if (argc > 1 && streq(argv[1], "--help")) {
        write_cstr("usage: tail [-n N] [FILE...]\n");
        exit_code(0);
    }

    if (argc > 2 && streq(argv[1], "-n")) {
        if (parse_u64(argv[2], &lines) != 0) {
            write_cstr("tail: invalid line count\n");
            exit_code(1);
        }
        first_path = 3;
    ...
    if (first_path >= argc) {
        exit_code(tail_fd(0, lines) == 0 ? 0 : 1);
    }

    for (uint64_t i = first_path; i < argc; i++) {
        if (tail_file(argv[i], lines) != 0) rc = 1;
    }
    exit_code(rc);
}
